    ...
    try {
      ...
      const reportAnalysis = sortedReports.map((r) => {
        ...
        Object.entries(r.metrics).forEach(([key, data]) => {
          ...
          const inOptimal =
            data.optimalMin !== null && data.optimalMax !== null
              ? data.value >= data.optimalMin && data.value <= data.optimalMax
              : null;
        ...
        return {
          ...
          suboptimalFindings: findings.filter(
            (f) => f.status === 'normal' && f.inOptimalRange === false
          ),
      ...
      keyMetrics.forEach((key) => {
        ...
        if (values.length >= 2) {
          trends[latestMetric.name] = {
            ...
            optimalRange:
              latestMetric.optimalMin !== null && latestMetric.optimalMax !== null
                ? `${latestMetric.optimalMin}–${latestMetric.optimalMax} ${latestMetric.unit}`
                : null,
      ...
      {summary ? (
        ...
        <div>
          <p className="text-sm text-muted-foreground mb-3">
            Get AI analysis of your {reports.length} reports with reference range context.
          </p>
          <button
            onClick={generateSummary}
            disabled={loading}
