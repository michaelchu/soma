    ...
    Object.entries(report.metrics).forEach(([key, data]) => {
      ...
      if (status !== 'normal') {
        allAbnormals.push({
          ...
          optimal: (data.optimalMin !== null && data.optimalMax !== null) ? `${data.optimalMin}–${data.optimalMax}` : null,
          status
        });
      }
    });
  ...
  if (allAbnormals.length === 0) {
    return (
      <div className="bg-green-50 dark:bg-green-950/30 rounded-xl p-4 border border-green-200 dark:border-green-900">
        <p className="text-sm text-green-700 dark:text-green-400">✓ All values normal across all reports!</p>
      </div>
    );
  }
    ...
    <div className="bg-card rounded-xl border overflow-hidden">
      <div className="px-4 py-3 bg-amber-50 dark:bg-amber-950/30 border-b border-amber-100 dark:border-amber-900">
        <h3 className="font-semibold text-amber-800 dark:text-amber-400 flex items-center gap-2">
          <AlertTriangle size={16} />Historical Abnormals ({allAbnormals.length})
        </h3>
      </div>
          ...
          <TableBody>
            {allAbnormals.map((item, idx) => (
              <TableRow key={idx}>
                ...
                <TableCell className="text-right text-muted-foreground text-xs">{item.range}</TableCell>
                <TableCell className="text-center">
                  <Badge
                    variant="outline"
                    className={item.status === 'high'
